...
export async function POST(request) {
  ...
  try {
    ...
    if (action === "delete") {
      const n = await cms.bulkDelete(user.email, ids);
      return NextResponse.json({ ok: true, count: n });
    }
    if (action === "publish") {
      const n = await cms.bulkSetStatus(user.email, ids, "published");
      return NextResponse.json({ ok: true, count: n });
    }
    if (action === "archive") {
      const n = await cms.bulkSetStatus(user.email, ids, "archived");
      return NextResponse.json({ ok: true, count: n });
    }
    if (action === "draft") {
      const n = await cms.bulkSetStatus(user.email, ids, "draft");
      return NextResponse.json({ ok: true, count: n });
    }
