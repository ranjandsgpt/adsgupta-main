...
# Public Feed API for main domain integration
@api_router.get("/articles/latest")
async def get_public_articles_feed(limit: int = 10):
    """
    ...
    articles_list = await cursor.to_list(length=limit)
...
async def get_redirect_uri():
    ...
    if not backend_url:
        raise HTTPException(status_code=500, detail="BACKEND_URL environment variable is required")
    return {
...
async def create_indexes():
    ...
    try:
        ...
        # Archives
        await db.archives_content.create_index("article_id", unique=True)
        await db.archives_content.create_index("slug", unique=True)
        await db.archives_content.create_index([("category", 1), ("publish_date", -1)])
        await db.archives_content.create_index("is_published")
