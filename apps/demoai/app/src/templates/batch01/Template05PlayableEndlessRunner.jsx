...
function Runner({ reducedMotion }) {
  ...
  const game = useRef({ y: 150, velocity: 0, obstacle: 420, score: 0 });
  ...
  return (
    <>
      <canvas ref={canvasRef} width="420" height="200" className="w-full rounded-2xl" aria-label={`Endless runner game. Score ${score}.`} />
      <div className="mt-4 flex gap-3">
        <button type="button" onClick={jump} className="min-h-12 flex-1 rounded-xl bg-cyan-400 px-5 font-black text-slate-950">Jump</button>
        <button type="button" onClick={() => setRunning((value) => !value)} className="flex min-h-12 min-w-12 items-center justify-center rounded-xl bg-white/10" aria-label={running ? 'Pause game' : 'Play game'}>
          {running ? <Pause /> : <Play />}
        </button>
      </div>
      <p className="mt-3 text-center text-sm font-bold" aria-live="polite">Score: {score} · Clear 3 gates to complete</p>
    </>
  );
}
