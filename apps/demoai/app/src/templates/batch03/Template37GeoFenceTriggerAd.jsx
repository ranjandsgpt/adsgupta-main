...
export default function Template37GeoFenceTriggerAd() {
  ...
  return (
    <section ref={viewRef} className="mx-auto max-w-md overflow-hidden rounded-3xl bg-slate-950 p-5 text-white shadow-2xl">
      ...
      <div className="relative my-4 flex min-h-36 items-center justify-center">
      ...
      {inside ? <button type="button" onClick={claim} disabled={claimed} className="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-400 px-4 font-black text-emerald-950 hover:bg-emerald-300 disabled:cursor-default disabled:hover:bg-emerald-400">{claimed && <Check size={19} />}{claimed ? 'Offer claimed — show at checkout' : 'Claim free cold brew'}</button> : <button type="button" onClick={trigger} className="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-white px-4 font-black text-slate-950 hover:bg-cyan-100"><LocateFixed size={19} /> Simulate arrival</button>}
    </section>
  );
}
