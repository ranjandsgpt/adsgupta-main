...
export default function Template43TickerSponsorshipStrip() {
  ...
  return (
    <aside ref={viewRef} className="mx-auto flex max-w-4xl items-stretch overflow-hidden rounded-xl bg-slate-950 text-white shadow-xl">
      <div className="flex shrink-0 items-center bg-emerald-400 px-4 text-xs font-black uppercase tracking-wider text-emerald-950">Energy now</div>
      <div className="relative flex min-h-14 min-w-0 flex-1 items-center overflow-hidden">
        <div className={`flex min-w-max items-center gap-8 px-5 text-sm font-bold ${paused || reducedMotion ? '' : 'animate-pulse'}`}>{ITEMS.map((item) => <span key={item} className="whitespace-nowrap">{item}</span>)}<button type="button" onClick={report} className="inline-flex min-h-11 items-center gap-1 rounded-lg px-3 text-emerald-300 underline underline-offset-4">Clean grid report <ArrowUpRight size={16} /></button></div>
      </div>
