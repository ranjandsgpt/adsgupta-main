  ...
  return (
    <NativeWidgetChrome label="Sponsored · demo form" title="Build your 4-week learning plan" onClose={() => dismiss('button')}>
      <div className="p-4">
        ...
        {step === 3 && <div role="status" className="mt-5 rounded-2xl bg-emerald-400/10 p-6 text-center text-emerald-200"><Check className="mx-auto mb-2" />Demo lead complete. No data left this browser.<button type="button" onClick={() => { setStep(0); setForm({ goal: '', email: '', consent: false }); emitTelemetry('click', { templateId: ID, target: 'restart-form' }); }} className="mx-auto mt-4 block min-h-11 rounded-xl border border-emerald-300/40 px-4 font-bold text-emerald-100 hover:bg-emerald-400/10">Restart demo</button></div>}
