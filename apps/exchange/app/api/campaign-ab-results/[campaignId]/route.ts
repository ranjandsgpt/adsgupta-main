...
export async function GET(request: NextRequest, { params }: { params: { campaignId: string } }) {
  ...
  const full = await sql`
    SELECT advertiser_name, advertiser, advertiser_email, contact_email FROM campaigns WHERE id = ${params.campaignId} LIMIT 1
  `;
  ...
  const payload = await buildAbResults(params.campaignId);
