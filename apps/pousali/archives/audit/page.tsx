...
import Header from '../../src/app/audit/components/Header';
import UploadPanel from '../../src/app/audit/components/UploadPanel';
import AuditProcessingPanel from '../../src/app/audit/components/AuditProcessingPanel';
import AuditSummaryBlock from '../../src/app/audit/components/AuditSummaryBlock';
import AuditTabs from '../../src/app/audit/components/AuditTabs';
import ExportBar from '../../src/app/audit/components/ExportBar';
import { FeedbackWidget } from '../../src/app/audit/components/FeedbackWidget';
import ReconciliationDiagnosticsPanel from '../../src/app/audit/components/ReconciliationDiagnosticsPanel';
import PrivacyNote from '../../src/app/audit/components/PrivacyNote';
import { AuditStoreProvider, useAuditStore } from '../../src/app/audit/context/AuditStoreContext';
import { GeminiReportProvider, useGeminiReport } from '../../src/app/audit/context/GeminiReportContext';
import { ExportProvider, useExport } from '../../src/app/audit/context/ExportContext';
import { ValidatedArtifactsProvider } from '../../src/app/audit/store/ValidatedArtifactsContext';
import { DualEngineProvider, useDualEngine } from '../../src/app/audit/dualEngine/dualEngineContext';
import { PipelineProvider, usePipeline, type PipelineStageId } from '../../src/app/audit/context/PipelineContext';
import { LearningProvider, useLearning } from '../../src/app/audit/learning/LearningContext';
import { parseReportsStreaming } from '../../src/app/audit/utils/reportParser';
import { normalizeToCsvFiles } from '../../src/app/audit/utils/xlsxToCsv';
import { runReportVerification } from '../../src/app/audit/utils/reportVerification';
import type { TabId } from '../../src/app/audit/tabs/useTabData';
import { PendingCopilotQuestionProvider } from '../../src/app/audit/context/PendingCopilotQuestionContext';
import { initUserSession, saveAuditResult } from '../../src/lib/userSession';
    ...
    (async () => {
      try {
        ...
        let store = await parseReportsStreaming(
          ...
          (stage, status, error) => {
            setStage(
              ...
              status === 'failed' ? 'failed' : status === 'running' ? 'running' : 'completed',
              error
            );
          }
        );
        ...
        while (attempt < MAX_PARSING_RETRIES - 1) {
          ...
          store = await parseReportsStreaming(csvFiles, () => {}, (stage, status) => {
            setStage(
              stage as PipelineStageId,
              status === 'failed' ? 'failed' : status === 'running' ? 'running' : 'completed'
            );
        ...
        setStage(
          ...
          verification.crossReport.passed ? 'completed' : 'failed',
          verification.crossReport.errors[0]
        );

        setStore(store);
        try {
          if (sessionRef.current && store.aggregatedMetrics) {
            await saveAuditResult(sessionRef.current.sessionId, sessionRef.current.userId, store.aggregatedMetrics);
          }
        } catch (e) {
          // eslint-disable-next-line no-console
  ...
  return (
    <div className="min-h-screen bg-[var(--color-surface)] text-[var(--color-text)]">
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-10 pb-6 pr-6 sm:pr-8 space-y-6">
        <UploadPanel onUploadComplete={handleUploadComplete} disabled={step === 'processing'} collapsed={step === 'dashboard'} />
        {step === 'processing' && <AuditProcessingPanel />}
...
export default function ArchivedAuditPage() {
  return (
    <AuditStoreProvider>
      <LearningProvider>
        <PipelineProvider>
