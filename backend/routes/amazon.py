...
async def amazon_oauth_callback(
    ...
    """
    frontend_url = os.environ.get("FRONTEND_URL", "https://ads-gupta-preview.preview.emergentagent.com")
    
    if error:
        logger.error(f"Amazon OAuth error: {error} - {error_description}")
        return {"redirect": f"{frontend_url}/tools?error={error}&message={error_description}"}
    ...
    return {"redirect": f"{frontend_url}/tools?amazon_connected=true"}
