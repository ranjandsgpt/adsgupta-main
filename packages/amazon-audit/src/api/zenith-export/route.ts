...
import { runCxoJudgeAgent } from '@adsgupta/amazon-audit/agents/cxoJudgeAgent';
import { runBrandIntelligence } from '@adsgupta/amazon-audit/agents/brandIntelligenceAgent';
import type { PremiumState, VerifiedMetric, VerifiedInsight } from '@adsgupta/amazon-audit/agents/zenithTypes';
import { setExportStatus } from '@adsgupta/amazon-audit/services/exportStatusStore';
import { writeCache } from '@adsgupta/amazon-audit/services/exportCache';
import { checkExportConsistency } from '@adsgupta/amazon-audit/services/exportConsistencyGuard';
import { verifyPptxOutput } from '@adsgupta/amazon-audit/agents/ExportVerificationAgent';
import { generateSlideContent, type SlideManifest } from '@adsgupta/amazon-audit/agents/ClaudeSlideDataAgent';
import type { AggregatedMetrics } from '@adsgupta/amazon-audit/lib/aggregateReports';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Slide 1 — Overview (cover + narrative)
    {
      ...
      slide.addText(`Generated ${new Date(premiumState.generatedAt).toLocaleDateString()} | ${getAuditBrandRuntime().siteUrl.replace(/^https?:\/\//, '')}`, {
        x: 0.8,
        y: 5.2,
        w: 8,
        h: 0.35,
